...
describe("EthernautTest", function () {

it("Shouldn't validate the level if user didn't pass it", async function () {
    // SETUP SCENARIO
    [owner, attacker] = await ethers.getSigners();

    // Implementation contract
    const MotorBikeFactory = await ethers.getContractFactory("MotorbikeFactory");
    const motorBikeFactory = await MotorBikeFactory.deploy();
    await motorBikeFactory.deployed();

    let tx = await motorBikeFactory.createInstance(owner.address);
    let receipt = await tx.wait();


    // Implementation contract
    const Engine = await ethers.getContractFactory("Engine");
    const engine = await Engine.attach(await motorBikeFactory.engine());

    // Proxy contract
    const Motorbike = await ethers.getContractFactory("Motorbike");
    let motorbike = await Motorbike.attach(await motorBikeFactory.motorbike());

    const boolean = await motorBikeFactory.validateInstance(motorbike.address, owner.address);
    expect(boolean).to.false;
})

it("Should let the user solve the level", async function () {
    // SETUP SCENARIO
    [owner, attacker] = await ethers.getSigners();
    ...
    expect((await proxyedMotorbike.upgrader())).to.equal(motorBikeFactory.address)
